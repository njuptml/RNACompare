...
function [alpha,primal,dual,gap,CPUtime,itercount] = IndefiniteSVR_ACCPM(K,labls,c,eps_SVR,rho,accuracy,info,maxiters,x)
...
notconverged=1;
% Begin loop
disp('Indefinite SVR (using ACCPM) MATLAB starting...');
while(notconverged)
...
%         disp(['eigUpateMult_mex failed at iteration ',num2str(iter),' with C=',num2str(c)]);
    ...
    D=D.*(D>0);
    if iter==0
        primal_lb=sum(labls.*x)-eps_SVR*sum(abs(x))-.5*((x'*V).^2)*D+rho*D'*D-2*rho*grad_traces'*D+rho*trace_K_K;
    else
        primal_lb=max(primal_lb,sum(labls.*x)-eps_SVR*sum(abs(x))-.5*((x'*V).^2)*D+rho*D'*D-2*rho*grad_traces'*D+rho*trace_K_K);
    end
    if mod(iter,info)==0 
        CPUtime=[CPUtime,toc];
        primal_temp=sum(labls.*x)-sum(abs(x))-.5*((x'*V).^2)*D+rho*D'*D-2*rho*grad_traces'*D+rho*trace_K_K;
        ...
        dual_temp=alpha_temp'*labls-eps_SVR*sum(abs(alpha_temp))-.5*alpha_temp'*K_opt*alpha_temp+rho*trace((K_opt-K)'*(K_opt-K));
    ...
    t=1;count=0;
    while sum([b_cutplane;c;c;-1*lowerbox(2:end);upperbox(2:end)]-[AF;F(1,:);-1*F(1,:);-1*eye(n-1);eye(n-1)]*(z+t*dirF)<=0)>0
        t=alpha*t;
    ...
    z=AnalyticCenterCG_mex([AF;F(1,:);-1*F(1,:)],[b_cutplane;c;c],lowerbox(2:end),upperbox(2:end),newtonMaxIters,newtonEpsilon,z,[AFhat;F(1,:).^2;F(1,:).^2]);
    x=F*z;
    if (iter >= maxiters)
        notconverged = 0;
    end;
