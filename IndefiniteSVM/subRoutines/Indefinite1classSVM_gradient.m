% Implementation of Projected Gradient Method for Indefinite1classSVM
%
...
function [alpha,primal,dual,gap,CPUtime,itercount] = Indefinite1classSVM_gradient(K,nu,rho,accuracy,info,maxiters,x,stepsize)
disp('Indefinite one-class SVM (using projected gradient) MATLAB starting...');
...
%         disp(['eigUpateMult_mex failed at iteration ',num2str(iter),' with C=',num2str(c)]);
    ...
    if mod(iter,info)==0    
        CPUtime=[CPUtime,toc];
        D=D.*(D>0);primal=[primal;-.5*((x'*V).^2)*D+rho*D'*D-2*rho*grad_traces'*D+rho*trace_K_K];
        ...
        dual=[dual;-.5*alpha_temp'*K_opt*alpha_temp+rho*trace((K_opt-K)'*(K_opt-K))];
    ...
    if (notconverged)
        ...
        x = projcutsimplex5(x,ones(n,1),zeros(n,1),ones(n,1)/nu/n,1); % project the new point back to the feasible region
    end
    iter = iter + 1;
end;
alpha=x;
